...
#[AsCommand(
    name: 'app:seed',
    description: 'Seed the database with initial data (safe for production)',
)]
class SeedCommand extends Command
{
    ...
    {
        $this->addOption('force', null, InputOption::VALUE_NONE, 'Purge existing data before seeding');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->purgeTables($io, ['article', 'avis', 'faq_item', 'logement', 'user']);

        $io->section('Seeding...');

        $this->seedUser($io);
        $this->seedLogements($io);
        $this->seedArticles($io);
        $this->seedFaq($io);
        $this->seedAvis($io);
    ...
    private function purgeTables(SymfonyStyle $io, array $tables): void
    {
        ...
        if ($platform instanceof AbstractMySQLPlatform) {
            ...
            foreach ($tables as $table) {
                $quoted = $conn->getDatabasePlatform()->quoteIdentifier($table);
                $conn->executeStatement("TRUNCATE TABLE {$quoted}");
        ...
        if ($platform instanceof PostgreSQLPlatform) {
            $quotedTables = array_map(
                fn (string $table): string => $conn->getDatabasePlatform()->quoteIdentifier($table),
                $tables,
            );
            $conn->executeStatement('TRUNCATE TABLE ' . implode(', ', $quotedTables) . ' RESTART IDENTITY CASCADE');
            foreach ($tables as $table) {
                $io->writeln("  Truncated $table");
            }
        ...
        throw new \RuntimeException(sprintf('Unsupported database platform for purge: %s', $platform::class));
    }

    private function seedUser(SymfonyStyle $io): void
    {
        $user = new User();
        ...
        $io->writeln('  Created admin user');
    }

    private function seedLogements(SymfonyStyle $io): void
    {
        $logements = [
            [
                'slug'          => 'le-presquile',
        ...
        foreach ($logements as $data) {
            ...
            $l->setNom($data['nom']);
            $l->setType($data['type']);
            $l->setQuartier($data['quartier']);
            $l->setVoyageurs($data['voyageurs']);
            $l->setChambres($data['chambres']);
            $l->setNote($data['note']);
            $l->setAvis($data['avis']);
            $l->setOccupation($data['occupation']);
            $l->setRevenus($data['revenus']);
            $l->setImgIndex($data['imgIndex']);
            $l->setPhotos($data['photos']);
            $l->setDescription($data['description']);
            $l->setEquipements($data['equipements']);
            $l->setPointsInteret($data['pointsInteret']);
            $this->em->persist($l);
        }
        $io->writeln('  Created ' . count($logements) . ' logements');
    }

    private function seedArticles(SymfonyStyle $io): void
    {
        $articles = [
            [
        ...
        foreach ($articles as $data) {
            ...
            $a->setDate($data['date']);
            $a->setReadTime($data['read']);
            $a->setImg($data['img']);
            $a->setContent($data['content']);
            $this->em->persist($a);
        }
        $io->writeln('  Created ' . count($articles) . ' articles');
    }

    private function seedFaq(SymfonyStyle $io): void
    {
        $items = [
        ...
        foreach ($items as $data) {
            ...
            $f->setPosition($data['position']);
            $f->setQuestion($data['question']);
            $f->setReponse($data['reponse']);
            $this->em->persist($f);
        }
        $io->writeln('  Created ' . count($items) . ' FAQ items');
    }

    private function seedAvis(SymfonyStyle $io): void
    {
        $avisList = [
            [
                'nom'          => 'Claire B.',
        ...
        foreach ($avisList as $data) {
            $av = new Avis();
            $av->setNom($data['nom']);
            $av->setRole($data['role']);
            $av->setTexte($data['texte']);
            $av->setNote($data['note']);
            $av->setProprietaire($data['proprietaire']);
            $av->setPosition($data['position']);
            $this->em->persist($av);
        }
        $io->writeln('  Created ' . count($avisList) . ' avis');
    }
}
